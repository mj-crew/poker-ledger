...
// total (largest-remainder). items: [{id, weight}]. Returns { id: cents }.
export function allocateProrata(items, total) {
  const out = {};
  for (const i of items) out[i.id] = 0;
  const totalW = items.reduce((s, i) => s + i.weight, 0);
  if (totalW <= 0 || total <= 0) return out;
  const parts = items.map((i) => { const exact = (i.weight / totalW) * total; const cents = Math.floor(exact); return { id: i.id, cents, frac: exact - cents }; });
