...
export default async function expensesRoutes(app) {
  ...
  app.post("/expenses/:id/approve", { preHandler: [app.authenticate, requireCap("settlement.lock")] }, async (req) => {
    await query("UPDATE expenses SET status='approved' WHERE id=$1", [Number(req.params.id)]);
    return { ok: true };
  });
  app.post("/expenses/:id/reject", { preHandler: [app.authenticate, requireCap("settlement.lock")] }, async (req) => {
